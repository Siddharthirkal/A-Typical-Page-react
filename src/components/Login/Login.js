...
const Login = (props) => {
  ...
  const [passwordState, dispatachPassword] = useReducer(passwordReducer, {
    value: "",
    isValid: null,
  });
  ...
  const passwordChangeHandler = (event) => {
    dispatachPassword({ type: 'USER_INPUT', val: event.target.value});
  ...
  const validatePasswordHandler = () => {
    dispatachPassword({ type: 'INPUT_BLUR'});
  };
